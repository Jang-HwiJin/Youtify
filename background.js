  ...
  async getAccessToken() {
    try {
      ...
      if (
        ...
        Date.now() < result.token_expires_at
      ) {
  ...
  async authorizeUser() {
    return new Promise(async (resolve, reject) => {
      ...
      if (this.clientId === "YOUR_SPOTIFY_CLIENT_ID") {
        reject(
          new Error(
            "Please update the SPOTIFY_CLIENT_ID in manifest.json with your actual Spotify app Client ID",
          ),
        );
        return;
      }
      ...
      chrome.identity.launchWebAuthFlow(
        ...
        async (responseUrl) => {
          if (chrome.runtime.lastError) {
            ...
            reject(
              new Error(
                `Authorization failed: ${chrome.runtime.lastError.message}. Make sure you've added ${redirectURL} to your Spotify app's redirect URIs.`,
              ),
            );
            return;
          }
  ...
  const handleAsync = async () => {
    try {
      if (request.action === "addToSpotify") {
        const result = await handleAddToSpotify(request.songInfo);
        sendResponse(result);
        return;
      }
      ...
      if (request.action === "createPlaylist") {
        ...
        const playlist = await spotifyAPI.createPlaylist(request.name, request.description);
        sendResponse({ success: true, playlist });
        return;
      }

      if (request.action === "authorize") {
        const token = await spotifyAPI.authorizeUser();
        sendResponse({ success: true, token });
        return;
      }

      if (request.action === "callOllama") {
        console.log("Calling local API with:", request.title, request.channelName);

        const response = await fetch('http://localhost:4000/api/parse-song', {
          method: 'POST',
...
async function handleAddToSpotify(songInfo) {
  try {
    ...
    const { target_playlist } = await chrome.storage.sync.get([
      "target_playlist",
    ]);
